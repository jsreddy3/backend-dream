...
class DreamRepository(ABC):
    ...
    # segments
    @abstractmethod
    async def create_segment(self, user_id: UUID, segment: AudioSegment, session: AsyncSession) -> AudioSegment: ...
    @abstractmethod
    async def get_segment(self, user_id: UUID, did: UUID, sid: UUID, session: AsyncSession) -> Optional[AudioSegment]: ...
    @abstractmethod
    async def delete_segment(self, user_id: UUID, did: UUID, sid: UUID, session: AsyncSession) -> Optional[AudioSegment]: ...
    @abstractmethod
    async def update_segment_transcript(self, user_id: UUID, did: UUID, sid: UUID, transcript: str, session: AsyncSession) -> Optional[AudioSegment]: ...
    @abstractmethod
    async def update_segment_transcription_status(self, user_id: UUID, did: UUID, sid: UUID, status: str, session: AsyncSession) -> Optional[AudioSegment]: ...
